...
const defaultSkills = () => new Set(SKILLS.slice(0, 2).map((s) => s.id))

export function SavingsDemo() {
  const [modelByRole, setModelByRole] = useState<Record<string, string>>(defaultModels)
  const [loadedSkills, setLoadedSkills] = useState<Set<string>>(defaultSkills)
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-muted-foreground">
          Why skill stubs beat pasting instructions inline — deterministic demo math, not a
          live install.
        </p>
              ...
              {ROLES.map((role) => (
                ...
                >
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setModelByRole((prev) => ({ ...prev, [role.id]: v }))}
                  >
